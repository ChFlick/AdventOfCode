...
const dists = overlaps.map(node =>
    firstCoords.findIndex(v => v[0] === node[0] && v[1] === node[1]) +
    secondCoords.findIndex(v => v[0] === node[0] && v[1] === node[1])
);

// plus two as both also traverse over the start coord
console.log(Math.min(...dists) + 2);
