...
fun main(args: Array<String>) {
    val inputs = """p=<-3770,-455,1749>, v=<-4,-77,53>, a=<11,7,-9>
...
p=<2218,-821,794>, v=<312,-113,114>, a=<-23,9,-7>""".split('\n')

    var particles = inputs.map {
        val posVars = it.substring(it.indexOf("p=<") + 3, it.indexOf(">")).split(",")
        val velVars = it.substring(it.indexOf("v=<") + 3, it.indexOf(">", it.indexOf("v=<"))).split(",")
        val accVars = it.substring(it.indexOf("a=<") + 3, it.indexOf(">", it.indexOf("a=<"))).split(",")
        Particle(Vec3d(posVars[0].toDouble(), posVars[1].toDouble(), posVars[2].toDouble()),
                Vec3d(velVars[0].toDouble(), velVars[1].toDouble(), velVars[2].toDouble()),
                Vec3d(accVars[0].toDouble(), accVars[1].toDouble(), accVars[2].toDouble()))
    }

    // V1
//    repeat(10000, {
//        particles.forEach { it.compute() }
//    })
//
//    val distances = particles.map { it.distanceToZero() }
//    println(distances)
//    val minPart = particles.find { it.distanceToZero() == distances.min() }
//    println(particles.indexOf(minPart))


    //V2
    repeat(10000, {
        val doublePositions = particles.groupingBy { it.pos }.eachCount().filter { it.value > 1 }.keys
        if(doublePositions.isNotEmpty()) {
            particles = particles.filter {
                doublePositions.indexOf(it.pos) == -1
            }
        }
        particles.forEach { it.compute() }
    })
...
class Particle(var pos: Vec3d, var vel: Vec3d, var acc: Vec3d){
    override fun equals(other: Any?): Boolean {
        return pos == (other as Particle).pos
    }
    ...
    fun distanceToZero(): Int {
        return (Math.abs(pos.x) + Math.abs(pos.y) + Math.abs(pos.z)).toInt()
    }
}